...
def patient_SQLsearch():
    ...
    labelFrame.place(relx=0.1, rely=0.3, relwidth=0.8, relheigh=0.5)
    tk.Label(labelFrame,
             text="{:^10}{:^15}{:^25}{:^25}{:^10}".format('ID', 'Last Name', 'First Name', 'Room No', 'Department'),
             bg="black", fg="white").place(relx=0.07, rely=0.1)
...
def view_employees():
    ...
    try:
        ...
        for employee in cursor:
            ...
            tk.Label(labelFrame, text="{: <10}{: ^20}{: ^20}{: ^30}{: ^25}".format(str(employee[0]),
                                                                                    str(employee[1]), str(employee[2]), str(employee[3]), str(employee[4])),
                     bg="black", fg="white").place(relx=0.07, rely=y)
            y += 0.1
    except sqlite3.Error as err:
        tk.messagebox.showerror(title="Error", message=f"An error has occurred. {err}.")
...
def view_patients():
    ...
    tk.Label(labelFrame, text="{:^5}{:^20}{:^15}{:^20}{:^15}{}".format('ID', 'Name', 'Department', 'Room', 'Doctor', 'Check-in Date'),
          bg="black", fg="white").place(relx=0.07, rely=0.1)
    tk.Label(labelFrame, text="----------------------------------------------------------------------------------------", bg="black", 
    ...
    try:
        ...
        for patient in cursor:
            ...
            if doctorName:
                tk.Label(labelFrame, text="{:^2}{:^20}{:^25}{:^18}{:^25}{}".format(str(patient[0]), (patient[2] + ", " + patient[1]),
                                                                                patient[3], patient[4], str(doctorName[0]), patient[6]),
                    bg='black', fg="white").place(relx=0.07, rely=y)
            else:
                tk.Label(labelFrame, text="{:^2}{:^20}{:^25}{:^18}{:^25}{}".format(str(patient[0]), (patient[2] + ", " + patient[1]),
                                                                                patient[3], patient[4], "N/A", patient[6]),
                    bg='black', fg="white").place(relx=0.07, rely=y)
            y += 0.1
    except sqlite3.Error as err:
